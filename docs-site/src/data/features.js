...
export const FEATURES = [
  ...
  },
  {
    ...
    note: {
      es: 'Cifrado de extremo a extremo: la clave viaja en el fragmento de la URL —la parte tras la almohadilla, que los navegadores no envían nunca al servidor— así que la entrega el QR y el túnel solo transporta bytes opacos. El PIN tampoco cruza en claro: el emparejamiento ocurre dentro del socket ya cifrado, sin cookies ni rutas HTTP. El límite, dicho claro: quien sirve la página es quien podría alterar el JavaScript que descifra, así que esto protege de que se lea y se registre, no de un intermediario activamente hostil; para eso hay que no tener intermediario. Un PIN de seis dígitos con cinco intentos, y al quinto fallo se cierra la sesión entera en vez de fallar ese intento. Echar a alguien termina su sesión pero no le prohíbe volver: para eso está rotar el PIN, que no afecta a quien ya está dentro. El móvil trae su propio menú de botones —paneles, pestañas, git, docker, ficheros, música, paleta— porque un teclado de móvil no puede teclear un acorde con Alt y porque conducir la capa leader en remoto haría aparecer un menú en la pantalla del escritorio, que es la de otra persona. Lo que no es una celda no viaja: imágenes inline, el QR de la transferencia óptica y la onda del reproductor son texturas y salen como huecos. Una sola ventana comparte a la vez.',
      en: 'End-to-end encrypted: the key travels in the URL fragment \u2014 the part after the hash, which browsers never send to a server \u2014 so the QR delivers it and the tunnel carries nothing but opaque bytes. The PIN does not cross in the clear either: pairing happens inside the already-encrypted socket, with no cookies and no HTTP route. The limit, plainly: whoever serves the page is whoever could alter the JavaScript that decrypts, so this defends against reading and logging rather than against an actively hostile middle \u2014 for that you need no middle at all. Six digits with five attempts, and the fifth wrong one ends the whole session rather than that attempt. Dropping a viewer ends their session but does not ban them; rotating the PIN is what does, and it leaves anyone already connected alone. The phone carries its own menu of buttons \u2014 panes, tabs, git, docker, files, music, palette \u2014 because a phone keyboard cannot type a chord with Alt in it, and because driving the leader layer remotely would raise a menu on the desk\u2019s screen, which is somebody else\u2019s screen. Anything that is not a cell does not travel: inline images, the optical-transfer QR and the player\u2019s waveform are textures and arrive as holes. One window shares at a time.',
    },
  },
  {
