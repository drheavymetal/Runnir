...
export const FEATURES = [
  {
    ...
    natural: {
      es: 'runnir reproduce TIDAL él mismo, no abre otra aplicación. El panel tiene cinco fuentes a la izquierda —búsqueda, cola, favoritos, álbumes y listas— y lo que contengan en el centro. Una sola búsqueda devuelve pistas, álbumes, artistas y listas a la vez, y cada fila viene coloreada por el nivel que TIDAL ofrece de verdad: dorado para hi-res, verde para lossless, gris para con pérdidas. Eso se lee antes de darle al play, y no es un detalle — el campo obvio del catálogo dice «lossless» hasta para lo que se sirve a 24/192, así que el color sale de las etiquetas, que sí distinguen. Abajo, la barra de progreso; arriba, la onda; y un spinner mientras baja el primer segmento, porque ese hueco de un segundo parecía un cuelgue. El ratón funciona: un clic selecciona, el segundo sobre lo mismo actúa.',
      en: 'runnir plays TIDAL itself rather than opening another application. The panel has five sources down the left — search, queue, favourites, albums and playlists — and what they hold in the middle. One search returns tracks, albums, artists and playlists together, and every row is coloured by the tier TIDAL really offers: gold for hi-res, green for lossless, grey for lossy. You read that before pressing play, and it is not a detail — the obvious catalogue field says "lossless" even for what is served at 24/192, so the colour comes from the tags, which do tell them apart. Along the bottom, the progress bar; in the corner, the wave; and a spinner while the first segment is fetched, because that one-second gap looked like a hang. The mouse works: one click selects, a second on the same thing acts.',
    },
    keys: [
      'Leader N N',
      ...
      { es: 'H L o Tab · cambiar entre las fuentes y la lista', en: 'H L or Tab · move between the sources and the list' },
    ],
    config: [
  ...
  },
  {
    ...
    example: 'runnir @ transfer --path ~/notes.md --fps 25 --color',
    note: {
      es: 'Unos 22 KB/s con los valores por defecto (2953 bytes por código, que es un QR versión 40 exacto, en color y a los códigos por segundo que la máquina consiga pintar, hasta 30). Todos esos números están medidos contra un móvil real, no razonados: el fps se equivocó tres veces antes de quedar en automático, y el techo de 30 es donde satura el receptor, que lee unos 9 códigos por segundo. Texto, configs y diffs son segundos; un megabyte, cosa de un minuto. El fichero se gzipea antes cuando eso ayuda. El formato es el de decimen-optical-transfer (MIT), portado byte a byte, así que un receptor de decimen guardado hace meses también lee lo que emite runnir — y cada baldosa del mosaico es un QR normal, así que sigue leyéndolo aunque no sepa nada de mosaicos. Con --color cada código lleva otro encima: rojo y verde pintan el QR normal de siempre, y el azul un segundo frame del mismo stream. Leído como brillo —que es lo que hace cualquier decodificador— el primero sigue siendo un QR corriente, así que no se añadió nada al formato y un receptor que no sepa de color lee la mitad del stream a toda velocidad. Duplica lo que sale y también lo que hay que decodificar, y aun así midió +73% (22,5 KB/s contra 13): o sea que el cuello no era decodificar, sino cuántos códigos rinde cada captura. Va encendido por defecto, y --color 0 lo apaga.',
      en: 'About 22 KB/s at the defaults (2953 bytes a code, which is exactly a version-40 QR, in colour, at whatever rate the machine can paint up to 30). Every one of those numbers is measured against a real phone rather than reasoned: the frame rate was wrong three times before it became automatic, and the ceiling of 30 is where the receiver saturates at about 9 codes a second. Text, configs and diffs are seconds; a megabyte is about a minute. The file is gzipped first when that helps. The format is decimen-optical-transfer\u2019s (MIT), ported byte for byte, so a decimen receiver saved months ago reads what runnir sends too \u2014 and every tile of the mosaic is an ordinary QR, so it still reads it knowing nothing about mosaics. With --color each code carries another one on top: red and green paint the same ordinary QR, blue carries a second frame of the same stream. Read as brightness — which is what every decoder does — the first one is still a perfectly ordinary code, so nothing was added to the format and a receiver that knows nothing about colour reads half the stream at full speed. It doubles what goes out and what has to be decoded, and still measured +73% (22.5 KB/s against 13) — so the constraint was never decoding, it was how many codes each capture yields. On by default, and --color 0 turns it off.',
    },
  },
  {
    ...
    natural: {
      es: 'Leader R Shift+P llena la ventana con un QR y un PIN. Lo escaneas, tecleas los seis dígitos y tienes la terminal en el móvil: los paneles, la barra de pestañas y el panel que esté abierto, con sus colores y el cursor donde está. Desde ahí se escribe, se toca y se hace scroll. El panel con foco llena la pantalla del móvil —cuatro paneles en un escritorio son cuatro columnas legibles, y en un móvil son cuatro ilegibles de las que tres le roban sitio a la que estás leyendo— y el resto de la ventana sigue ahí, alrededor, a un botón de distancia. Cambiar de panel desde el móvil mueve el encuadre, así que se navega de panel en panel a pantalla completa.',
      en: 'Leader R Shift+P fills the window with a QR and a PIN. Scan it, type the six digits, and the terminal is on your phone: the panes, the tab bar and whatever panel is open, in their real colours with the cursor where it is. From there you type, tap and scroll. The focused pane fills the phone\u2019s screen \u2014 four panes on a desk are four readable columns, on a phone they are four unreadable ones with three of them stealing room from the one being read \u2014 and the rest of the window is still there, around it, one button away. Moving focus from the phone moves the frame, so panes are walked one full screen at a time.',
    },
    keys: [
      'Leader R Shift+P',
    ...
    note: {
      es: 'Cifrado de extremo a extremo: la clave viaja en el fragmento de la URL —la parte tras la almohadilla, que los navegadores no envían nunca al servidor— así que la entrega el QR y el túnel solo transporta bytes opacos. El PIN tampoco cruza en claro: el emparejamiento ocurre dentro del socket ya cifrado y cada conexión recibe un desafío propio que hay que citar, de modo que un frame capturado no sirve en otra conexión. Cinco PIN fallidos cierran la sesión entera, no ese intento. Echar a alguien le cierra el socket —no basta con quitarlo de una lista, porque entonces conserva el teclado— y rotar el PIN es lo que le impide volver. Dónde está el límite, dicho claro: **un móvil emparejado tiene el alcance de quien está sentado al teclado**, porque puede teclear; la puerta es el PIN y pasada la puerta no hay valla interior. Y quien sirve la página podría alterar el JavaScript que descifra, así que esto protege de que se lea y se registre, no de un intermediario activamente hostil; para eso hay que no tener intermediario. Todo lo anterior salió de una ronda de auditoría con revisores de solo lectura: la primera versión tenía un PIN forzable sin límite y saltable con un replay, y un guardián que se esquivaba con Ctrl+M. El móvil trae su propio menú de botones —paneles, pestañas, git, docker, ficheros, música, paleta— porque un teclado de móvil no puede teclear un acorde con Alt y porque conducir la capa leader en remoto haría aparecer un menú en la pantalla del escritorio, que es la de otra persona. Lo que no es una celda no viaja: imágenes inline, el QR de la transferencia óptica y la onda del reproductor son texturas y salen como huecos. Una sola ventana comparte a la vez.',
      en: 'End-to-end encrypted: the key travels in the URL fragment \u2014 the part after the hash, which browsers never send to a server \u2014 so the QR delivers it and the tunnel carries nothing but opaque bytes. The PIN does not cross in the clear either: pairing happens inside the already-encrypted socket and every connection is given a challenge it must quote, so a captured frame is worthless on another connection. Five wrong PINs end the whole session, not that attempt. Dropping a viewer shuts their socket \u2014 removing a row from a list is not enough, because they keep the keyboard \u2014 and rotating the PIN is what stops them coming back. Where the limit is, plainly: **a paired phone has the reach of somebody sitting at the keyboard**, because it can type; the door is the PIN, and past the door there is no inner fence. And whoever serves the page could alter the JavaScript that decrypts it, so this defends against reading and logging rather than against an actively hostile middle \u2014 for that you need no middle at all. All of the above came out of an audit round with read-only reviewers: the first version had a PIN that was brute-forceable without limit and bypassable by replay, and a guardian that Ctrl+M stepped around. The phone carries its own menu of buttons \u2014 panes, tabs, git, docker, files, music, palette \u2014 because a phone keyboard cannot type a chord with Alt in it, and because driving the leader layer remotely would raise a menu on the desk\u2019s screen, which is somebody else\u2019s screen. Anything that is not a cell does not travel: inline images, the optical-transfer QR and the player\u2019s waveform are textures and arrive as holes. One window shares at a time.',
    },
  },
  {
  ...
  },
  {
    ...
    note: {
      es: 'La capa iluminada necesita keycaps con leyenda translucida para valer de algo: con keycaps opacos el LED ilumina el contorno de la tecla y no la letra, asi que se ve la zona pero no que tecla es (medido sobre un Moonlander, con 33 teclas, con 8, y con esas 8 a brillo maximo). Los avisos de estado no tienen ese problema, porque no piden identificar ninguna tecla. No requiere Keymapp abierto ni kontroll: runnir escribe en /dev/hidrawN, localizado por su descriptor de informe (usage page 0xFF60), y los permisos los da la regla udev que ZSA ya distribuye. Keymapp tiene que haber corrido una vez para que exista su base de datos con el layout. RUNNIR_ZSA_DEBUG=1 dice en que paso se rinde.',
      en: 'The lit layer needs shine-through keycaps to be worth anything: with opaque caps the LED lights the gap around the cap rather than the legend, so you see a region and not which key it is (measured on a Moonlander with 33 keys, with 8, and with those 8 at maximum brightness). The ambient signals do not have that problem, since they ask you to identify no key at all. Needs neither Keymapp open nor kontroll: runnir writes to /dev/hidrawN, found by its report descriptor (usage page 0xFF60), and the permissions come from the udev rule ZSA already ships. Keymapp has to have run once so that its database of the flashed layout exists. RUNNIR_ZSA_DEBUG=1 says which step gave up.',
    },
  },
  {
    key: 'repo-verbs', section: 'distinctive', status: 'shipped',
    title: { es: 'Los verbos reales de este repo', en: 'The real verbs of this repo' },
    natural: {
      es: 'Leader O V aprende con qué se trabaja de verdad un repositorio, a partir de los comandos que ACERTARON ahí, y te los ofrece de vuelta: abres un repo que no conoces y la ventana ya sabe cómo se compila, se prueba y se despliega. Enter deja uno en el prompt (nunca lo ejecuta), X olvida todo lo aprendido de ese repo.',
      en: 'Leader O V learns what a repository is actually worked with from the commands that SUCCEEDED there, and offers them back: open a repo you do not know and the window already knows how it is built, tested and deployed. Enter puts one at the prompt (never runs it), X forgets everything learned about that repo.',
    },
    keys: ['Leader O V'],
    palette: 'Repo verbs',
    config: [
      { k: 'verbs.enabled', v: 'false', d: { es: 'Aprender los verbos de cada repositorio', en: 'Learn each repository’s verbs' } },
    ],
    note: {
      es: 'La línea de privacidad se aplica al CAPTURAR, no al mostrar. curl -H "Authorization: Bearer …", scp de un volcado de cliente o psql con una contraseña en la URL son cosas que la gente teclea: la cabeza es el verbo y la cola es privada. Filtrar al mostrar significaría que el secreto ya está en disco cuando alguien piensa en ello. Dos palabras, nunca tres: cargo a secas no dice si el repo se compila, se prueba o se publica, así que una herramienta conocida conserva su subcomando — pero solo cuando la siguiente palabra es de verdad un subcomando de un vocabulario fijo. La heurística tentadora («una palabra suelta tras una herramienta es un subcomando») convierte python train.py en un verbo con el nombre de fichero de alguien, y ssh cloudmax en uno con un nombre de máquina. Viene apagado, y nada se guarda nunca dentro del repositorio.',
      en: 'The privacy line is enforced at CAPTURE, not at display. curl -H "Authorization: Bearer …", an scp of a client dump, psql with a password in the URL — these are things people type, and the head is the verb while the tail is private. Filtering at display would mean the secret is already on disk by the time anyone thinks about it. Two words, never three: bare cargo says nothing about whether this repo is built, tested or published, so a known tool keeps its subcommand — but only when the next word really is a subcommand from a fixed vocabulary. The tempting heuristic (“a bare word after a tool is a subcommand”) turns python train.py into a verb containing somebody’s filename and ssh cloudmax into one containing a hostname. Off by default, and nothing is ever stored inside the repository.',
    },
  },
  {
    key: 'map', section: 'distinctive', status: 'shipped',
    title: { es: 'El mapa, y el salvapantallas', en: 'The map, and the screensaver' },
    natural: {
      es: 'Leader M aleja toda la VENTANA — todas las pestañas, no solo la que estás mirando — y dibuja cada panel como una tarjeta con su estado, el último comando que corrió y la cola de lo que imprimió. J/K mueven, Enter va a ese panel (cambiando de pestaña si está en otra), Esc vuelve. Las tarjetas conservan la geometría de cada pestaña y las pestañas se apilan hacia abajo, en vez de recolocarse en una rejilla ordenada: una sesión se reconoce por su forma, y un mapa que la reordena te obliga a reaprenderla cada vez. Contesta una pregunta que no contesta nada más en runnir: qué está corriendo en cualquier parte de esta ventana. La puesta al día es de la pestaña activa; esto no. Y hace de salvapantallas: llueven runas Elder Futhark por delante y por detrás de las tarjetas, y la hora está tallada en el centro de la ventana. Los estados se releen cada segundo, así que lo que miras de pasada es lo que está pasando ahora.',
      en: 'Leader M zooms the whole WINDOW out — every tab, not just the one you are looking at — and draws each pane as a card carrying its state, the command it last ran and the tail of what that printed. J/K move, Enter goes to that pane (switching tab if it is in another one), Esc comes back. Cards keep each tab’s own geometry and tabs stack downwards rather than re-flowing into a neat grid: a session is recognised by its shape, and a map that rearranges it makes you re-learn it every time. It answers a question nothing else in runnir does — what is running anywhere in this window. The catch-up is the active tab; this is not. And it doubles as a screensaver: Elder Futhark rains in front of and behind the cards, and the time is carved in the middle of the window. The readings are retaken every second, so what you glance at is what is happening now.',
    },
    keys: ['Leader M', { es: 'J/K mover · Enter abrir · Esc volver', en: 'J/K move · Enter open · Esc back' }],
    palette: 'Map',
    config: [
      { k: 'behaviour.screensaver_after_secs', v: '0', d: { es: 'Sale solo tras este tiempo sin teclear. 0 nunca', en: 'Comes up by itself after this long with nothing typed. 0 never' } },
    ],
    note: {
      es: '«Sin teclear» significa que ninguna tecla ha llegado a un panel — el mismo reloj que usa la puesta al día. El foco de la ventana no se consulta, en ninguna de las dos direcciones: una ventana con foco en otro monitor no es atención, y que el puntero cruce otra ventana no es ausencia. Cuando sale solo, CUALQUIER tecla lo quita y esa tecla no hace nada más: se puso ahí sin que se lo pidieras, así que la pulsación iba dirigida a lo que tapaba. La fuente de las runas viaja dentro del binario, así que esto funciona en una máquina sin nada instalado. El lienzo infinito con desplazamiento libre sigue SIN construir a propósito: un panel es un pseudo-terminal y su tamaño se mide en filas y columnas, no en píxeles, así que alejarse «de verdad» obligaría a redimensionar todos los programas de la sesión.',
      en: '“Nothing typed” means no keystroke has reached a pane — the same clock the catch-up uses. Window focus is not consulted in either direction: a focused window on a second monitor is not attention, and the pointer crossing another window is not absence. When it puts itself up, ANY key dismisses it and that key does nothing else: it got there unasked, so the keystroke was meant for whatever it covered. The rune font travels inside the binary, so this works on a machine with nothing installed. The infinite canvas with free panning is deliberately still NOT built: a pane is a pseudo-terminal and its size is rows and columns rather than pixels, so zooming out “for real” would mean resizing every program in the session.',
    },
  },
  {
    key: 'catch-up', section: 'distinctive', status: 'shipped',
    title: { es: 'La ventana te pone al día', en: 'The window catches you up' },
    natural: {
      es: 'Vuelves después de un rato y Leader U te da UNA línea por panel en vez de seis scrollbacks. Las ordena por lo que más te reclama: esperándote, luego fallado, luego palabra vigilada, luego corriendo, luego terminado. Los empates se rompen por id de panel, porque una lista que se reordena entre dos vistazos hay que leerla dos veces. Un panel que no tiene nada que decir no dice nada — cuatro líneas de «no pasó nada» son peor que no tener la función — salvo que esté bloqueado esperándote, que merece decirse aunque nada se haya movido.',
      en: 'Come back after a while and Leader U gives you ONE line per pane instead of six scrollbacks, ranked by what wants you most: waiting on you, then failed, then a watched word, then running, then done. Ties break by pane id, because a list that reorders between two glances is one you have to read twice. A pane with nothing to say says nothing — four lines of “nothing happened” are worse than no feature — unless it is blocking on you, which is worth saying even when nothing moved.',
    },
    keys: ['Leader U'],
    palette: 'Catch up',
    note: {
      es: '«Fuera» no es el foco de la ventana. El reloj corre desde la última tecla que llegó a un panel, que es lo único que runnir sabe con certeza: una ventana enfocada en un segundo monitor no es atención, y que el puntero cruce otra ventana no es ausencia. Tras un minuto de eso se marca el contador de comandos de cada panel, y la puesta al día informa solo de lo que pasó desde esa marca.',
      en: '“Away” is not window focus. The clock runs from the last keystroke that reached a pane, which is the one thing runnir knows for sure: a focused window on a second monitor is not attention, and the pointer crossing another window is not absence. After a minute of that, every pane’s command counter is marked, and the catch-up reports only what moved past its mark.',
    },
  },
  {
    key: 'war-room', section: 'distinctive', status: 'shipped',
    title: { es: 'La sala de guerra', en: 'The war room' },
    natural: {
      es: 'Leader W W abre una pestaña montada alrededor de un despliegue: un panel vigilando docker compose ps, un logs -f por cada servicio, y el propio despliegue escrito en el prompt sin lanzar. No pregunta nada — el fichero compose ya dice de qué está hecho el proyecto, así que runnir lo lee (buscando hacia arriba hasta la raíz del repositorio). Nada de lo que abre hace otra cosa que mirar: ni up -d, ni restart, ni pull. Una ventana que se ordena sola es una comodidad; una que despliega sola es un accidente esperando testigo. Leader W Q la desmonta.',
      en: 'Leader W W opens a tab arranged around a deploy: a docker compose ps watch, one logs -f pane per service, and the deploy itself staged at the prompt, unfired. It asks nothing — the compose file already lists the services, so runnir reads it, walking up to the repository root. Nothing it opens does anything but watch: no up -d, no restart, no pull. A window that arranges itself is a convenience; one that deploys by itself is an accident waiting for a witness. Leader W Q takes it down.',
    },
    keys: ['Leader W W', 'Leader W Q'],
    palette: 'War room',
    note: {
      es: 'El desmontaje conserva lo que tocaste: cada panel recuerda si alguna tecla llegó a él, y el cierre solo quita los que abrió la sala Y en los que nadie escribió. El compose se parsea a mano en vez de añadir una dependencia de YAML, porque lo que hace falta es un nivel de claves bajo services:, y el parseo es deliberadamente conservador — lo ambiguo se descarta, ya que un panel abierto para ports o pgdata es una sala que miente sobre el proyecto.',
      en: 'Teardown keeps what you touched: every pane records whether a keystroke ever reached it, and the close only removes panes the room opened AND nobody typed in. The compose file is hand-parsed rather than pulling in a YAML crate, because what is needed is one level of keys under services:, and the parse is deliberately conservative — anything ambiguous is dropped, since a pane opened for ports or pgdata is a room that lies about the project.',
    },
  },
  {
    key: 'tactile-pipes', section: 'distinctive', status: 'shipped',
    title: { es: 'Tuberías táctiles', en: 'Tactile pipes' },
    natural: {
      es: 'Agarra el bloque de salida de un comando con el BOTÓN CENTRAL, suéltalo sobre otro panel: la salida se escribe a un fichero privado y su ruta queda escrita en el prompt de ese panel. La tubería que compones después de ejecutar, con la mano. Un soltar propone y nunca ejecuta: la ruta se deja en la línea para que una tecla la confirme, porque un gesto que ejecuta algo es un resbalón de ratón que ejecuta algo — y este parte de una salida que no necesariamente has leído.',
      en: 'Grab a command’s output block with the MIDDLE button and drop it on another pane: the output is written to a private file and its path is staged at that pane’s prompt. The pipe you compose after running, with your hand. A drop proposes and never executes — the path is left on the command line for a key to confirm, because a gesture that runs something is a mouse slip that runs something, and this one starts from output you have not necessarily read.',
    },
    keys: [{ es: 'Arrastre con el botón central entre paneles', en: 'Middle-button drag between panes' }],
    note: {
      es: 'Siempre a través de un fichero, sea del tamaño que sea: escribir el texto en la shell ejecutaría cada línea como un comando, que es lo contrario de «como entrada estándar». El fichero es 0600 en el directorio de ejecución del usuario, la misma ruta privada que usa el volcado de scrollback, porque la salida de un comando lleva secretos con la frecuencia suficiente como para suponer que los lleva. El CLIC central sigue pegando la selección primaria como siempre: la pulsación solo arma el gesto, y el pegado ocurre al soltar cuando el puntero no llegó a moverse (12 px). Soltar sobre un panel con un programa a pantalla completa se rechaza — escribir una ruta dentro de vim no es una tubería.',
      en: 'Always through a file, whatever the size: typing the text into a shell would run each line as a command, which is the opposite of “as stdin”. The file is 0600 in the per-user runtime dir, the same private-write path the scrollback dump uses, because command output holds secrets often enough to assume it does. Middle CLICK still pastes the primary selection exactly as before: the press only arms the gesture, and the paste happens on release when the pointer never really moved (12 px). Dropping on a pane running a full-screen app is refused — typing a path into vim is not a pipe.',
    },
  },
  {
    key: 'bundled-fonts', section: 'appearance', status: 'shipped',
    title: { es: 'Fuentes dentro del binario', en: 'Fonts inside the binary' },
    natural: {
      es: 'runnir lleva sus fuentes dentro: JetBrains Mono Nerd (las cuatro caras) y Noto Sans Runic. Una máquina recién instalada se ve como la terminal fue diseñada, sin instalar nada. Siempre como ÚLTIMO RECURSO: si tienes la fuente en el sistema gana la tuya, así que nadie que la haya reemplazado a propósito se ve sobrescrito, y nadie carga dos copias de los mismos contornos.',
      en: 'runnir carries its fonts inside: JetBrains Mono Nerd (all four faces) and Noto Sans Runic. A freshly installed machine looks the way the terminal was designed, with nothing installed. Always a LAST RESORT: if the system has the family, the system’s copy wins, so nobody who deliberately replaced it gets overridden and nobody carries two copies of the same outlines.',
    },
    note: {
      es: 'Las dos son casos distintos. La rúnica hay que embeberla: ninguna fuente monoespaciada de un sistema normal cubre el bloque Runic, así que sin ella la lluvia del mapa serían cuadraditos — algo roto, no distinto. La familia por defecto es el caso débil y conviene decirlo: runnir ya cae a la monoespaciada que haya, así que el texto se dibuja igual. La Nerd es multi-licencia (contornos OFL-1.1 de JetBrains, iconos con CC BY de Font Awesome, MIT de Devicons y Octicons, y otras); assets/fonts/NOTICE.md las lista, porque CC BY exige atribución y el aviso tiene que viajar con los ficheros.',
      en: 'They are two different cases. The runic face has to be bundled: no monospace face on a normal system carries the Runic block, so without it the map’s rain would be empty boxes — broken, not merely different. The default family is the weaker case and worth saying so: runnir already falls back to whatever monospace a system has, so text renders either way. The Nerd font is multi-licence (OFL-1.1 outlines from JetBrains, icons under Font Awesome’s CC BY, MIT from Devicons and Octicons, and others); assets/fonts/NOTICE.md lists them, because CC BY requires attribution and the notice has to travel with the files.',
    },
  },
  {
