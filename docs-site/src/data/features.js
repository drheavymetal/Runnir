...
export const FEATURES = [
  ...
  },
  {
    ...
    note: {
      es: 'Unos 75 KB/s con los valores por defecto (2953 bytes por frame, que es un QR versión 40 exacto, a 30 fps), y el doble en una ventana ancha: el código se dimensiona por el lado corto, así que en 16:9 cabe un segundo al lado con exactamente la misma nitidez, y el panel lo pone solo. Texto, configs y diffs son segundos; un megabyte, unos diez. El fichero se gzipea antes cuando eso ayuda. El formato es el de decimen-optical-transfer (MIT), portado byte a byte, así que un receptor de decimen guardado hace meses también lee lo que emite runnir — y cada baldosa del mosaico es un QR normal, así que sigue leyéndolo aunque no sepa nada de mosaicos.',
      en: 'About 75 KB/s at the defaults (2953 bytes a frame, which is exactly a version-40 QR, at 30 fps), and double that on a wide window: a code is sized by the short axis, so a 16:9 window has room for a second one beside it at exactly the same sharpness, and the panel puts it there by itself. Text, configs and diffs are seconds; a megabyte is closer to ten. The file is gzipped first when that helps. The format is decimen-optical-transfer\u2019s (MIT), ported byte for byte, so a decimen receiver saved months ago reads what runnir sends too \u2014 and every tile of the mosaic is an ordinary QR, so it still reads it knowing nothing about mosaics.',
    },
  },
  {
