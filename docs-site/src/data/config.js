...
export const CONFIG_GROUPS = [
  {
    group: '[transfer]',
    rows: [
      { k: 'fps', v: '10', d: { es: 'Códigos por segundo. El 10 está MEDIDO contra un móvil real: 15, 24 y 30 salieron peor, y 5 peor en el otro sentido. El techo lo pone la cámara, y un código necesita más que un intervalo de captura — necesita que la mano se quede quieta, que el enfoque asiente y que la exposición no pille un cambio a medias. Capturas repetidas del mismo código no cuestan nada (el fountain descarta un número de bloque que ya tiene), así que enseñarlo de más es casi gratis y enseñarlo de menos se pierde entero.', en: 'Codes per second. The 10 is MEASURED against a real phone: 15, 24 and 30 were all worse, and 5 was worse the other way. The ceiling is the camera, and a code needs more than one capture interval — it needs the hand to settle, autofocus to lock and an exposure that does not straddle a change. Repeat captures of the same code cost nothing (the fountain discards a sequence number it already has), so showing one for too long is nearly free and showing it for too little is total loss.' } },
      ...
      { k: 'color', v: 'false', d: { es: 'Lleva un segundo código en el color del primero: rojo y verde pintan el QR de siempre, en blanco y negro, y el azul otro frame del mismo stream. Leído como brillo —que es lo que hace cualquier decodificador— el primero sigue siendo un QR corriente, así que no se añade nada al formato y un receptor que no sepa de color lee la mitad del stream a toda velocidad. Duplica lo que sale y también lo que el receptor tiene que decodificar, así que gana cuando el móvil no es ya el cuello de botella (pocas capturas tiradas en su línea de métricas) y no compra nada cuando lo es. Se puede probar por emisión con runnir @ transfer --color, sin tocar este fichero.', en: 'Carry a second code in the colour of the first: red and green paint the usual black-and-white QR, blue carries another frame of the same stream. Read as brightness — which is what every decoder does — the first one is still an ordinary QR, so nothing is added to the format and a receiver that knows nothing about colour reads half the stream at full speed. It doubles what goes out and what the receiver has to decode, so it wins when the phone is not already the bottleneck (few dropped captures on its metrics line) and buys nothing when it is. Try it per stream with runnir @ transfer --color, without touching this file.' } },
    ],
  },
  {
    group: '[tidal]',
